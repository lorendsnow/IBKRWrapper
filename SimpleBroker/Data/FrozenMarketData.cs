...
{
    /// <summary>
    /// Represents "frozen" market data, which is a snapshot of the market data at the time of
    /// the request (or at the close if the request was made after hours)
    /// </summary>
    ...
    public class FrozenMarketData(int reqId, Contract contract)
    {
        ...
        )
        {
            ...
            switch ((StandardTickIds)e.Data.Field)
            {
                case StandardTickIds.BidOptionComputation:
                    BidGreeks = e.Data.Value.ToBrokerOptionGreeks();
                    break;

                case StandardTickIds.AskOptionComputation:
                    AskGreeks = e.Data.Value.ToBrokerOptionGreeks();
                    break;

                case StandardTickIds.LastOptionComputation:
                    LastGreeks = e.Data.Value.ToBrokerOptionGreeks();
                    break;

                case StandardTickIds.ModelOptionComputation:
                    ModelGreeks = e.Data.Value.ToBrokerOptionGreeks();
                    break;
        ...
        /// <summary>
        /// Returns a string representation of the <see cref="FrozenMarketData"/>
        /// </summary>
        /// <returns><see cref="string"/></returns>
        public override string ToString()
        {
            return $$"""FrozenMarketData { Symbol: {{Contract.Symbol}}, BidPrice: {{BidPrice}}, """
                + $$"""BidSize: {{BidSize}}, AskPrice: {{AskPrice}}, AskSize: {{AskSize}}, """
                + $$"""LastPrice: {{LastPrice}}, LastSize: {{LastSize}}, High: {{High}}, Low: """
                + $$"""{{Low}}, Volume: {{Volume}}, CloseTick: {{CloseTick}}, OpenTick: """
                + $$"""{{OpenTick}}, Timestamp: {{TimeStamp}}, BidGreeks: {{BidGreeks}}, """
                + $$"""AskGreeks: {{AskGreeks}}, LastGreeks: {{LastGreeks}}, ModelGreeks: """
                + $$"""{{ModelGreeks}}, Halted: {{Halted}} }""";
        }
    }
}
