...
namespace SimpleBroker
{
    ...
    /// <remarks>
    ///     Includes an event that is triggered and emits the new bar when received from IBKR.
    ///     Subscribe to <see cref="NewBarEvent"/> to receive new bars.
    /// </remarks>
    ...
    )
    {
        ...
        public void HandleNewBar(object? sender, RealTimeBarEventArgs e)
        {
            ...
            RealTimeBar bar = new(e.Time, e.Open, e.High, e.Low, e.Close, e.Volume, e.WAP, e.Count);
