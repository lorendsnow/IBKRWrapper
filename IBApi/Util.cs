...
namespace IBApi
{
    public static class Util
    {
        ...
        public static int StringCompare(string lhs, string rhs) =>
            NormalizeString(lhs).CompareTo(NormalizeString(rhs));
        ...
        public static string IntMaxString(int value) =>
            value == int.MaxValue ? string.Empty : string.Empty + value;

        public static string LongMaxString(long value) =>
            value == long.MaxValue ? string.Empty : string.Empty + value;

        public static string DoubleMaxString(double value) => DoubleMaxString(value, string.Empty);

        public static string DoubleMaxString(double value, string def) =>
            value != double.MaxValue ? value.ToString(NumberFormatInfo.InvariantInfo) : def;

        public static string DecimalMaxString(decimal value) =>
            value == decimal.MaxValue ? string.Empty : string.Empty + value;

        public static string DecimalMaxStringNoZero(decimal value) =>
            value == decimal.MaxValue || value == 0 ? string.Empty : string.Empty + value;

        public static string UnixSecondsToString(long seconds, string format) =>
            new DateTime(1970, 1, 1, 0, 0, 0)
                .AddSeconds(Convert.ToDouble(seconds))
                .ToLocalTime()
                .ToString(format);

        public static string UnixMilliSecondsToString(long milliSeconds, string format) =>
            new DateTime(1970, 1, 1, 0, 0, 0)
                .AddMilliseconds(Convert.ToDouble(milliSeconds))
                .ToLocalTime()
                .ToString(format);

        public static string FormatDoubleString(string str) =>
            string.IsNullOrEmpty(str) ? string.Empty : DoubleMaxString(double.Parse(str));
        ...
        public static decimal StringToDecimal(string str) =>
            ...
            && !str.Equals("-9223372036854775808")
                ? decimal.Parse(str, NumberFormatInfo.InvariantInfo)
                : decimal.MaxValue;

        public static decimal GetDecimal(object value) =>
            Convert.ToDecimal(((IEnumerable)value).Cast<object>().ToArray()[0]);

        public static bool IsVolOrder(string orderType) =>
            orderType.Equals("VOL") || orderType.Equals("VOLATILITY") || orderType.Equals("VOLAT");

        public static bool IsPegBenchOrder(string orderType) =>
            orderType.Equals("PEG BENCH") || orderType.Equals("PEGBENCH");

        public static bool IsPegMidOrder(string orderType) =>
            orderType.Equals("PEG MID") || orderType.Equals("PEGMID");

        public static bool IsPegBestOrder(string orderType) =>
            orderType.Equals("PEG BEST") || orderType.Equals("PEGBEST");
